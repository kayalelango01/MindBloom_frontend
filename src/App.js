/* =========================================
   MindBloom – App.js
   Full React app — connected to Django backend
...
function calcStreakFromBackend(moods) {
  if (!moods || !moods.length) return 0;
  const dates = [...new Set(moods.map(m => m.date))].sort().reverse();
  let streak = 0;
...
function Nav({ page, setPage, dark, setDark, onLogout }) {
...
function Home({ setPage, user }) {
  ...
  useEffect(() => {
    ...
    getJournal().then(res => { if (Array.isArray(res)) setJournals(res); });
  }, []);

  const streak   = calcStreakFromBackend(moods);
  ...
  return (
    <div className="page">
      <div className="home-hero">
        <p className="home-greeting">{greet}{user ? `, ${user.username}` : ""}</p>
...
function CheckIn({ toast }) {
  ...
  useEffect(() => {
    ...
    getMoods().then(res => {
      if (Array.isArray(res)) setRecentMoods(res);
    });
  }, []);

  const toughMoods = ["Stressed", "Anxious"];
  const pastMoods  = recentMoods.filter(m => m.date !== todayStr()).slice(0, 3).map(m => m.mood);
  const showAlert  = (
    pastMoods.length >= 2 &&
    pastMoods.slice(0, 2).every(m => toughMoods.includes(m)) &&
    selected &&
    toughMoods.includes(selected)
  );

  const saveMood = async () => {
    if (!selected) { toast("Select a mood first"); return; }
    if (saving)    return;
    ...
    if (res?._status === 201) {
      ...
      console.error("logMood error:", res);
    }
  };

  const doSpin = () => {
    if (spunToday) { toast("Already spun today! Come back tomorrow 🎡"); return; }
    if (spinning)  return;
    setSpinning(true);
    ...
    <div className="page">
      ...
      {selected && moodEntry && (
        <Card className="suggestion-card" style={{
          ...
          {!alreadyToday && (
            <Btn onClick={saveMood} disabled={saving}>
      ...
      <Card className="spin-section">
        ...
        <div className="spin-wheel-wrap">
          <div
            ...
            style={{
              ...
              transition: spinning ? "transform 1.2s cubic-bezier(.17,.67,.27,1.2)" : "transform 0.3s ease",
            }}
          >
            🌻
          </div>
        ...
        {spunToday && !spinResult && (
          <div className="spin-result">You've already received your positivity boost today! Come back tomorrow 🌻</div>
        )}
          ...
          {entries.map(e => (
            <Card key={e.id} className="card-sm entry-card">
              ...
              <div className="entry-preview">
                {e.content && e.content.length > 160 ? e.content.slice(0, 160) + "…" : e.content}
              </div>
...
function Insights() {
  ...
  useEffect(() => {
    Promise.all([getMoods(), getJournal()]).then(([m, j]) => {
      if (Array.isArray(m)) setMoods(m);
      if (Array.isArray(j)) setJournals(j);
      setLoading(false);
    });
  }, []);
  ...
  if (loading) {
    return (
      <div className="page">
        ...
        <p className="empty-state" style={{ marginTop: "3rem" }}>Loading insights… 🌱</p>
      </div>
    );
  }
    ...
    <div className="page">
      ...
      <Card className="chart-card">
        ...
        <div className="wb-wrap">
          {[{ label: "Weekdays", s: wdS }, { label: "Weekends", s: weS }].map(({ label, s }) => (
            <div key={label}>
              <div className="wb-label">{label}</div>
              <div className="wb-bars">
                <div className="wb-bar-item">
                  <div className="wb-bar" style={{ height: `${pct(s.tough, s.total) * 0.7}px`, background: "linear-gradient(to top, var(--pink), #FFB6C8)" }}/>
                  <div className="wb-pct">{pct(s.tough, s.total)}%</div>
                  <div className="wb-sublabel">stressed</div>
                </div>
                <div className="wb-bar-item">
                  <div className="wb-bar" style={{ height: `${pct(s.pos, s.total) * 0.7}px`, background: "linear-gradient(to top, var(--lav), #C4B5FD)" }}/>
...
function Emergency({ toast }) {
  ...
  const saveContact = async () => {
    ...
    const res = await apiSaveContact({ name: name.trim(), phone: phone.trim() });
  ...
  const removeContact = () => {
    setContact(null);
    toast("Contact removed from view");
  };
    ...
    <div className="page">
      ...
      <div className="emergency-cards">
        ...
        <Card>
          ...
          {loadingC ? (
            <p style={{ fontSize: "0.85rem", color: "var(--text3)" }}>Loading…</p>
          ) : contact ? (
...
function Profile({ toast, user, setUser }) {
  ...
  const doLogin = async () => {
    ...
    const res = await apiLogin({ username: email, password: pw });
  ...
  const doSignup = async () => {
    ...
    if (res && res.id) {
      setUser(res);
      toast("Account created! Welcome to MindBloom 🌻");
    } else {
      const errMsg = res?.username?.[0] || res?.password?.[0] || res?.error || "Signup failed ❌";
      toast(errMsg);
    }
  };
  ...
  if (user) {
    return (
      <div className="page page-narrow">
        ...
        <Btn variant="ghost" full onClick={doLogout}>Logout</Btn>
      </div>
    );
  }
      ...
      {tab === "login" ? (
        <>
          ...
          <div className="form-group">
            ...
            <InputField placeholder="your_username" value={email} onChange={e => setEmail(e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">Password</label>
            <InputField type="password" placeholder="••••••••" value={pw} onChange={e => setPw(e.target.value)} />
          </div>
          <Btn full onClick={doLogin}>Login</Btn>
        ...
        <>
          ...
          <div className="form-group">
            <label className="form-label">Username</label>
            <InputField placeholder="choose_a_username" value={email} onChange={e => setEmail(e.target.value)} />
          </div>
          <div className="form-group">
          ...
          <Btn full onClick={doSignup}>Create Account</Btn>
          <p className="auth-switch">
...
function App() {
  ...
  if (!authReady) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh" }}>
        <p style={{ color: "var(--text3)", fontSize: "1rem" }}>Loading MindBloom… 🌻</p>
      </div>
    );
  }
  ...
  if (!user && page !== "profile") {
    return (
      <>
        <Profile toast={toast} user={user} setUser={setUser} />
  ...
  const pageMap = {
    ...
    profile:   <Profile   toast={toast} user={user} setUser={setUser} />,
  };
